...
const CreateWingo = async(req, res) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        await connection.execute(sql, ['20240524300', arr[i], timeNow]);
        const sql_1 = "INSERT INTO wingo SET period = ?, game = ?, amount = 0, status = 0, time = ?";
        await connection.execute(sql_1, ['20240524300', arr[i], timeNow]);
    }
...
const Create5D = async(req, res) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        await connection.execute(sql, ['20240524300', '23521', arr[i], timeNow]);
        const sql_1 = "INSERT INTO 5d SET period = ?, result = ?, game = ?, status = 0, time = ?";
        await connection.execute(sql_1, ['20240524300', '0', arr[i], timeNow]);
    }
...
const CreateK3 = async(req, res) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        await connection.execute(sql, ['20240524300', '235', arr[i], timeNow]);
